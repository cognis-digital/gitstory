"""Native cognis-connect emit for gitstory — forward findings to any platform.

Maps gitstory's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    gitstory ... --format json | gitstory-emit --to stix
    gitstory-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('type', None)
        out.pop('scope', None)
        out.pop('breaking', None)
        out.pop('footer', None)
        out['title'] = rec.get('subject', '')
        out['description'] = rec.get('body', '')
        out['tags'] = [rec.get('type', '').lower(), rec.get('scope', '').lower()]
        out['severity'] = 'info'
        out['type'] = 'commit'
