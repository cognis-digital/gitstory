...
**gitstory** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `gitstory` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
gitstory ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
gitstory ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
gitstory ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `gitstory` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="gitstory") for rec in records]   # records = your JSON output
